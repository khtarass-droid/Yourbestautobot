# Your Best Auto Bot

Telegram-бот для публікації оголошень у канал Your Best Auto.

## Що робить

- приймає до 80 фото/відео;
- перше фото/відео публікує в канал;
- решту медіа публікує в групу коментарів альбомами по 10;
- додає кнопки: Кредит / Локація / Viber / TikTok;
- формує красивий опис авто.

## Команди

- `/start` — запуск;
- `/new` — нове оголошення;
- `/done` — завершити завантаження фото/відео;
- `/publish` — опублікувати;
- `/cancel` — скасувати.

## Render

Build Command:

```bash
pip install -r requirements.txt
```

Start Command:

```bash
python bot.py
```

Environment Variables:

```env
BOT_TOKEN=токен від BotFather
CHANNEL_ID=-1002681585680
DISCUSSION_GROUP_ID=-1004435080882
CREDIT_URL=https://ref.best/Your_best_autoLV
LOCATION_URL=https://maps.google.com/?q=49.22654,23.81327
VIBER_URL=viber://chat?number=%2B380676755121
TIKTOK_URL=https://www.tiktok.com/@yourbestauto
MAX_MEDIA=80
```

## Важливо

Бот має бути адміністратором каналу і групи коментарів.
У каналі потрібно дати право публікувати повідомлення.
У групі коментарів потрібно дати право надсилати медіа.
